...
const App = () => {
  ...
  const modifyBlog = async (blogObject) => {
    try {
      ...
      setBlogs(blogs.map(blog => modifiedBlog.id === blog.id ? { ...blog, likes: modifiedBlog.likes } : blog).sort((a,b) => b.likes-a.likes))
    } catch (exception) {
    ...
    <div>
      ...
      {user === null ?
        ...
        <div>
          ...
          {blogs.map(blog =>
            <Blog key={blog.id} blog={blog} modifyBlog={modifyBlog} deleteBlog={deleteBlog}/>
          )}
        </div>
      }
    </div>
